...
import { createDynamicTable, updateDynamicTable } from './dynamicTable.service';

// CREATE ONE
const createController = async (req: Request, res: Response) => {
  const body = req.body as createSchemaType['body'];
  const params = req.params as createSchemaType['params'];

  const findProject = await db.project.findOne({
    slug: params.projectSlug,
  });

  if (!findProject) throw new AppError('project not found', { status: 404 });
...
const updateController = async (req: Request, res: Response) => {
  ...
  const updatedResult = await updateDynamicTable(
    { id: params.id, projectId: findProject.id },
    body,
  );
